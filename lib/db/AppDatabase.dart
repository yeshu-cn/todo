...
class AppDatabase {
  ...
  List<Task> bindData(List<Map<String, dynamic>> result) {
	...
	Future updateTask(Task task) async {
  	var db = await _getDb();
		return db.rawUpdate(
				"UPDATE ${Task.tblTask} SET ${Task.dbTitle} = ?, ${Task.dbStatus} = ? WHERE ${Task.dbStatus} = ?", [task.title, task.tasksStatus, task.id]);
	}

	Future deleteTask(int taskId) async {
  	var db = await _getDb();
  	return db.transaction((txn) async {
  		await txn.rawDelete("DELETE FROM ${Task.tblTask} WHERE ${Task.dbId} = $taskId;");
		});
	}

	Future insertTask(String title) async {
  	var db = await _getDb();
  	return db.rawInsert('INSERT INTO ${Task.tblTask} (${Task.dbTitle}, ${Task.dbStatus}) VALUES ("$title", ${TaskStatus.PENDING.index});');
	}
}
